...
describe('html', () => {
    ...
    it('should render nested src 3 levels', () => {
        const deep = html`<ul>
            ${html`<li>${html`<span>${html`sample`}</span>`}</li>`}
        </ul>`
    ...
    it('should handle custom events attribute with web components', () => {
        ...
        const btn = html`<my-button onactive="${clickMock}"
            >click me</my-button
        >`
    ...
    it('should ignore inline event if its one of its prop', () => {
        ...
        expect(() => html`<one-comp one="${2}"></one-comp>`.render(document.body)).not.toThrowError()
        expect(() => html`<two-comp one="${2}"></two-comp>`.render(document.body)).toThrowError(
            'handler for event "one" is not a function. Found "2".'
        )
    })

    it('should handle ref directive', () => {
        const btn = html`<button ref="btn">
            ${html`<div ref="div">
    ...
    describe('should work with "repeat" helper', () => {
        ...
        it('with object value and WC return', () => {
            ...
            const el = html`${repeat(
                () => items,
                (n) => html`<test-component val="${n}"></test-component>`
            )}`
        ...
        it('with array of values as html instance based on object', () => {
            class TodoItem extends HTMLElement {
                ...
                connectedCallback() {
                    ...
                    this.temp = html` <div class="todo-item">
                        <div class="details">
                            <h3>${this.#name}</h3>
                            ${when(
                                this.#description,
        ...
        it('with state', () => {
            ...
            const Todo = html`<ul>
                ${repeat(todos, (todo: any) => html`<li>${todo}</li>`)}
            </ul>`
        ...
        it('when item changes', () => {
            ...
            >([], updateMock)
            const todoItem = (todo: any) => html`<div id="${todo.id}">${todo.name} <span>${todo.status}</span></div>`
            const todoList = html`${repeat(todos, todoItem)}`;
            
            html`<div>${todoList}</div>`.render(document.body)
    ...
    it('should ignore values between tag and attribute', () => {
        ...
        const btn = html`<button ${disabled}></button>`
