...
export default ({ page, nextPage, prevPage, docsMenu }: PageComponentProps) =>
    DocPageLayout(
        ...
        html`
            ${Heading(page.name)}
            <p>
                Having a global store can come in handy. There are many
                solutions out there you can use but let's entertain the idea you
                can create a simple store using the
                <a href="./state-values#state">state</a> function.
            </p>
            ...
            <p>
                This is a simple file store that handles all the CRUD operations
                and only exposes the necessary.
            </p>
            <p>
