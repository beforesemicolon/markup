...
export default DocPageLayout(
    ...
    html`
        ...
        <p>
            If you have a state value and try to read a deep value it would look
            like so:
        </p>
        ${CodeSnippet(
        ...
        <p>
            The issue here is that when the <code>user</code> changes the DOM
            will not update because the template only react to states that are
            provided directly in the template. The above example a function is
            provided and the template cannot see inside the function to see the
            state and subscribe to changes.
        </p>
        <p>
        ...
        <ul>
            <li>
                <strong>OBJECT</strong>: Any JavaScript object which value can
                be chained to access the value (Array, Object).
            </li>
            <li>
                <strong>KEYCHAIN_STRING</strong>: A string with dot separated
                keys.
            </li>
        </ul>
        ...
        <p>
            The <code>pick</code> helper second argument can be a dot-notation
            string that can go deep.
        </p>
        ${CodeSnippet(
        ...
        ${DocPrevNextNav({
            prev: {
                label: HelpersGroup.list[5].name,
                link: HelpersGroup.list[5].path,
            },
            next: {
