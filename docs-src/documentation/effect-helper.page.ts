...
export default DocPageLayout(
    ...
    html`
        ${Heading(page.name)}
        <p>
            When using <a href="./state-values">state</a> you will notice that
            only the part of the template dependent on the state will update
            with changes.
        </p>
        <p>
            For those situations you would want to perform "side effects" on the
            template, meaning, you want other part of the template to change
            with some state change.
        </p>
        <p>
        ...
        <p>
            The <code>effect</code> take comma-separated states and as the last
        ...
        ${DocPrevNextNav({
            prev: {
                label: HelpersGroup.list[0].name,
                link: HelpersGroup.list[0].path,
            },
            next: {
